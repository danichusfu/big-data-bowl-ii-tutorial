...
Let's test out how that works in a pipe scenario.

Since `x` is the first argument when we use the pipe `data` will be set to `x`. We can then fill in the next argument with `3`.
...
It is important to note that if the function has more than one argument and you would like to assign a value to an argument that is not the next one you can specify what you are doing with the name of the argument.
...
With that aside covered let's get to some Football!
